"""Convert CKIP BERT WS to CoreML — manual BERT, no HF dependency issues."""
...
# Minimal BERT for tracing (no HF dependency)
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        out = torch.matmul(weights, v).permute(0, 2, 1, 3).reshape(B, L, -1)
        return out
...
class BertLayer(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        h = torch.nn.functional.gelu(self.intermediate_dense(x))
...
def load_from_mlx_dir(task_dir):
    ...
    # Load from MLX safetensors (already mapped keys)
    weights = {}
    st_path = os.path.join(task_dir, "weights.safetensors")
...
print("1. Loading model from MLX weights...")
model, config = load_from_mlx_dir("../ckip_mlx/models/ws")
model.eval()
...
print("3. Converting to CoreML (dynamic seq 1-512)...")
